...
import type { MojoExecutableRegionAnalysisInput } from "./analyze.js";
import {
  mergeMojoErrorTypes,
...
export function executableRegionErrorTypes(
  ...
  walkSourceTree(root, input.source.ast, (node): void => {
    ...
    if (input.source.ast.is.IsPropertyAccessExpression(node)) {
      ...
      } else if (selection?.kind === "provider-union-property") {
        addNativeConversionError(selection.variants.some((variant) =>
          mojoConversionRaises(variant.readConversion)));
